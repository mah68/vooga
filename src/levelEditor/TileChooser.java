...
import sprites.SpriteClassFinder;
...
GraphicsBankChangeListener {
	...
	public TileChooser(GraphicsBank gfx, LevelEditorController controller,
			JFrame dialogOwner) {
		this(gfx);
	...
	public void createPropertiesDialog(JFrame dialogOwner) {
		...
		ArrayList<Class<?>> mySpriteClasses = finder.getClasses();
		ArrayList<String> classNames = new ArrayList<String>();
		
		for(Class<?> current : mySpriteClasses){
			classNames.add(current.getName().substring(8));
		}
		
		classList = new JComboBox(classNames.toArray());
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == applyBtn && propertyTile != null) {
			...
			propertyTile.className = (String)classList.getSelectedItem();
